...
namespace HospitalApp.Models
{
    ...
    {   
        ...
        // Returns the patient's derived diet tags using the app's shared diet vocabulary.
        public List<DietTag> DietTags
        {
            get
            {
                HashSet<DietTag> tags = new();

                if (IsDiabetic)
                {
                    tags.Add(DietTag.Diabetic);
                    tags.Add(DietTag.LowSugar);
                }

                if (HasKidneyDisease || HasLiverDisease || BpStatus is BloodPressureStatus.Elevated or BloodPressureStatus.HighStage1 or BloodPressureStatus.HighStage2 or BloodPressureStatus.HypertensiveCrisis)
                {
                    tags.Add(DietTag.LowSodium);
                }

                if (HasLiverDisease || CholesterolStatus is CholesterolLevel.BorderlineHigh or CholesterolLevel.High)
                {
                    tags.Add(DietTag.LowFat);
                    tags.Add(DietTag.LowCholesterol);
                }

                if (CholesterolStatus is CholesterolLevel.BorderlineHigh or CholesterolLevel.High || BpStatus is BloodPressureStatus.Elevated or BloodPressureStatus.HighStage1 or BloodPressureStatus.HighStage2 or BloodPressureStatus.HypertensiveCrisis)
                {
                    tags.Add(DietTag.HeartHealthy);
                }

                if (BmiCategory is BmiCategory.Overweight or BmiCategory.Obese or BmiCategory.SeverelyObese)
                {
                    tags.Add(DietTag.WeightLoss);
                    tags.Add(DietTag.HighFiber);
                }

                if (tags.Count == 0)
                {
                    tags.Add(DietTag.GeneralWellness);
                }
